...
class PlaylistSuggestion {
    constructor(client) {
        ...
        this.getTrackBasedSuggestions = async (track, userId, guildId, limit) => {
            try {
                ...
                let guildTracks = [];
                if (userId) {
                    const userData = await repo_1.MusicDB.getUserMusicHistory(userId);
                    if (userData && userData.songs)
                        userTracks = userData.songs.filter((s) => s && s.uri && s.title && s.author && s.uri !== track.uri);
                }
                if (guildId) {
                    const guildData = await repo_1.MusicDB.getGuildMusicHistory(guildId);
                    if (guildData && guildData.songs)
                        guildTracks = guildData.songs.filter((s) => s && s.uri && s.title && s.author && s.uri !== track.uri);
                }
        ...
        this.getGlobalSimilarTracks = async (track, limit) => {
            try {
                if (!track || !track.title || !track.author)
                    return [];
                const globalData = await repo_1.MusicDB.getGlobalMusicHistory();
                if (!globalData?.songs?.length)
                    return [];
        ...
        this.getUserTopRecommendations = async (userId, limit, existingTracks = []) => {
            try {
                if (!userId)
                    return [];
                const userSongs = await repo_1.MusicDB.getUserTopSongs(userId, limit * 2);
                if (!userSongs?.length)
                    return [];
        ...
        this.getGuildRecommendations = async (guildId, limit, existingTracks = []) => {
            try {
                if (!guildId)
                    return [];
                const guildSongs = await repo_1.MusicDB.getGuildTopSongs(guildId, limit * 2);
                if (!guildSongs?.length)
                    return [];
                const existingUris = new Set(existingTracks.filter((t) => t && t.uri).map((t) => t.uri));
        ...
        this.getSuggestionsFromUserTopSong = async (userId, guildId, limit = this.defaultLimit) => {
            try {
                const startTime = Date.now();
                const userTopSongs = await repo_1.MusicDB.getUserTopSongs(userId, 1);
        ...
        this.getSuggestions = async (userId, guildId, seedTrack, limit = this.defaultLimit) => {
            try {
                const suggestions = [];
        ...
        this.getGlobalRecommendations = async (limit, existingTracks = []) => {
            try {
                const existingUris = new Set(existingTracks.filter((t) => t && t.uri).map((t) => t.uri));
                const globalData = await repo_1.MusicDB.getGlobalMusicHistory();
                if (!globalData?.songs?.length)
        ...
        this.client = client;
        this.spotifyService = search_1.SpotifyService.getInstance(client);
    }
}

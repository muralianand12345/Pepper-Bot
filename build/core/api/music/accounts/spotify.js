...
class SpotifyAPIHandler {
    constructor(client) {
      ...
      `);
            ...
            const tokens = await new music_1.SpotifyManager(this.client).exchangeCodeForTokens(code);
            if (!tokens) {
                res.status(500).send(`
        <!DOCTYPE html>
        <html>
      ...
      `);
            ...
            const username = await new music_1.SpotifyManager(this.client).getSpotifyUsername(tokens.access);
            const saved = await new music_1.SpotifyManager(this.client).saveAccount(userId, tokens, username || undefined);
            if (!saved) {
                res.status(500).send(`
        <!DOCTYPE html>
        <html>

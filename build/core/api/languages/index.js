...
class LanguagesAPIHandler {
    constructor(client) {
        this.setupRoutes = () => {
            this.router.get('/', this.handleList);
            this.router.get('/:code', this.handleLanguage);
            this.router.use((_req, res) => res.status(404).json({ success: false, error: 'Unknown languages endpoint' }));
        };
        ...
        this.handleList = async (_req, res) => {
            try {
        ...
        this.handleLanguage = async (req, res) => {
            try {

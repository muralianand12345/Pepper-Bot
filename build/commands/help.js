...
const helpCommand = {
    ...
    execute: async (interaction, client) => {
        ...
        if (specificCommand) {
            ...
            if (!command) {
                const embed = responseHandler.createErrorEmbed(t('responses.help.command_not_found', {
            ...
            const commandEmbed = new discord_js_1.default.EmbedBuilder()
                ...
                .addFields([
                {
                    name: t('responses.help.cooldown'),
                    value: command.cooldown ? `${command.cooldown}s` : t('responses.help.no_cooldown'),
                    inline: true,
                },
                {
                    name: t('responses.help.permissions'),
                    value: command.owner ? t('responses.help.owner_only') : command.userPerms ? command.userPerms.join(', ') : t('responses.help.none'),
                    inline: true,
                },
            ])
                .setFooter({
                text: t('responses.help.command_footer'),
        ...
        const embed = new discord_js_1.default.EmbedBuilder()
            ...
            .setDescription(t('responses.help.description', {
            total: commands.length,
            prefix: '/',
        }))
            .setThumbnail(client.user?.displayAvatarURL() || '')
            .setFooter({
            text: t('responses.help.footer'),
            iconURL: client.user?.displayAvatarURL(),
        })
            .setTimestamp();
        if (categories.music.length > 0)
            embed.addFields([
                {
                    name: `🎵 ${t('responses.help.categories.music')} (${categories.music.length})`,
                    value: formatCommands(categories.music),
                    inline: false,
                },
            ]);
        if (categories.utility.length > 0)
            embed.addFields([
                {
                    name: `🔧 ${t('responses.help.categories.utility')} (${categories.utility.length})`,
                    value: formatCommands(categories.utility),
                    inline: false,
                },
            ]);
        if (categories.other.length > 0)
            embed.addFields([
                {
                    name: `📦 ${t('responses.help.categories.other')} (${categories.other.length})`,
                    value: formatCommands(categories.other),
                    inline: false,
                },
            ]);

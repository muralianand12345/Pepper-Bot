...
const suggestSongsCommand = {
    ...
    execute: async (interaction, client) => {
        ...
        if (musicCheck)
            return await interaction.reply({ embeds: [responseHandler.createErrorEmbed(t('responses.errors.music_disabled'), locale)], flags: discord_js_1.default.MessageFlags.Ephemeral, });
        if (!interaction.guild)
            return await interaction.reply({ embeds: [responseHandler.createErrorEmbed(t('responses.errors.server_only'), locale)], flags: discord_js_1.default.MessageFlags.Ephemeral, });
        ...
        try {
            ...
            if (!seedSong)
                return await interaction.editReply({ embeds: [responseHandler.createInfoEmbed(t('responses.suggest_songs.no_history'), locale)], });
            if (!recommendations || recommendations.length === 0)
                return await interaction.editReply({ embeds: [responseHandler.createInfoEmbed(t('responses.suggest_songs.no_recommendations', { song: seedSong.title || "Unknown" }), locale)], });
            ...
            const embed = new discord_js_1.default.EmbedBuilder()
                ...
                .setDescription(t('responses.suggest_songs.description', { song: format_1.default.truncateText(seedSong.title || "Unknown Title", 40), artist: seedSong.author || "Unknown Artist", spotifyCount, totalCount: recommendations.length }))
            ...
            const row = new discord_js_1.default.ActionRowBuilder().addComponents(new discord_js_1.default.ButtonBuilder()
                ...
                .setEmoji("▶️"), new discord_js_1.default.ButtonBuilder()
                    ...
                    try {
                        ...
                        if (i.customId === "play-recommendation-first" && recommendations.length > 0) {
                            ...
                            if (!topPick || !topPick.uri)
                                return await i.followUp({ embeds: [responseHandler.createErrorEmbed(buttonT('responses.suggest_songs.errors.invalid_track'), buttonLocale, true)], components: [responseHandler.getSupportButton(buttonLocale)], flags: discord_js_1.default.MessageFlags.Ephemeral });
                            ...
                            if (searchResult.tracks && searchResult.tracks.length > 0) {
                                ...
                                player.stop();
                                await i.followUp({ embeds: [responseHandler.createSuccessEmbed(buttonT('responses.suggest_songs.now_playing', { title: topPick.title || "Unknown Track", artist: topPick.author || "Unknown Artist" }), buttonLocale)], flags: discord_js_1.default.MessageFlags.Ephemeral, });
                            }
                        }
                            ...
                            if (!player.playing && !player.paused && player.queue.size > 0)
                                player.play();
                            await i.followUp({ embeds: [responseHandler.createSuccessEmbed(buttonT('responses.suggest_songs.added_tracks', { count: addedCount }), buttonLocale)], flags: discord_js_1.default.MessageFlags.Ephemeral, });
                        }
                            ...
                            if (!newSeedSong || !newRecommendations || newRecommendations.length === 0)
                                return await i.followUp({ embeds: [responseHandler.createInfoEmbed(buttonT('responses.suggest_songs.no_new_recommendations'), buttonLocale)], flags: discord_js_1.default.MessageFlags.Ephemeral, });
                            ...
                            const updatedEmbed = new discord_js_1.default.EmbedBuilder()
                                ...
                                .setDescription(buttonT('responses.suggest_songs.description', { song: format_1.default.truncateText(newSeedSong.title || "Unknown Title", 40), artist: newSeedSong.author || "Unknown Artist", spotifyCount: newSpotifyCount, totalCount: newRecommendations.length }))
                        ...
                        if (!i.replied && !i.deferred)
                            await i.reply({ embeds: [responseHandler.createErrorEmbed(t('responses.errors.general_error'), locale, true)], components: [responseHandler.getSupportButton(locale)], flags: discord_js_1.default.MessageFlags.Ephemeral, });
                    }
                });

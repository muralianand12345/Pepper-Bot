...
const langCommand = {
    ...
    data: new discord_js_1.default.SlashCommandBuilder()
        ...
        name: 'User',
        value: 'user',
        name_localizations: localizationManager.getCommandLocalizations('commands.language.options.scope.choices.user'),
    }, {
        name: 'Server',
        value: 'server',
        name_localizations: localizationManager.getCommandLocalizations('commands.language.options.scope.choices.server'),
    }, {
        name: 'Reset',
        value: 'reset',
    ...
    execute: async (interaction, client) => {
        ...
        try {
            if (scope === 'reset') {
                if (interaction.inGuild()) {
                    ...
                    if (interaction.memberPermissions?.has(discord_js_1.default.PermissionsBitField.Flags.ManageGuild))
                        await localeDetector.setGuildLanguage(interaction.guildId, null);
                }
                else {
                    await localeDetector.setUserLanguage(interaction.user.id, null);
                }
            ...
            if (!localeDetector.isLanguageSupported(language)) {
                ...
                const embed = responseHandler.createErrorEmbed(t('responses.language.unsupported', {
                    language,
            ...
            if (scope === 'user') {
                ...
                if (currentUserLang === language) {
                    ...
                    const embed = responseHandler.createInfoEmbed(t('responses.language.same_language', {

...
const validateLavalinkButtonInteraction = (interaction) => {
    return interaction.isButton() && LAVALINK_BUTTON_IDS.includes(interaction.customId);
};
const handleLavalinkButtonAction = async (interaction, client) => {
    try {
        ...
        switch (interaction.customId) {
            case 'refresh_lavalink_status': {
                ...
                if (userLavalink.lastError)
                    embed.addFields([{ name: t('responses.lavalink.fields.last_error'), value: userLavalink.lastError, inline: false }]);
            ...
            case 'remove_lavalink_quick': {
                ...
                await interaction.reply({ embeds: [confirmEmbed], components: [confirmButton], flags: discord_js_1.default.MessageFlags.Ephemeral });
                break;
            }
...
const event = {
    ...
    execute: async (interaction, client) => {
        if (!validateLavalinkButtonInteraction(interaction))
            return;

	...
	execute: async (interaction: discord.ChatInputCommandInteraction, client: discord.Client): Promise<void | discord.Message<boolean>> => {
		...
		if (musicCheck)
			return await interaction.editReply({
				embeds: [responseHandler.createErrorEmbed(t('responses.errors.music_disabled'), locale)],
			});
		if (!interaction.guild)
			return await interaction.editReply({
				embeds: [responseHandler.createErrorEmbed(t('responses.errors.server_only'), locale)],
			});
		...
		try {
			...
			if (!recommendations || recommendations.length === 0)
				return await interaction.editReply({
					embeds: [
						responseHandler.createInfoEmbed(
							t('responses.suggest_songs.no_recommendations', {
								song: seedSong.title || 'Unknown',
							}),
							locale
						),
					],
			...
			const embed = new discord.EmbedBuilder()
				...
				.setFooter({
					text: t('responses.suggest_songs.footer', {
				...
				collector.on('collect', async (i: discord.MessageComponentInteraction) => {
					try {
						...
						if (i.customId === 'play-recommendation-first' && recommendations.length > 0) {
							...
							await i.deferUpdate();
							const { seedSong: newSeedSong, recommendations: newRecommendations } = await suggestionEngine.getSuggestionsFromUserTopSong(
								interaction.user.id,

...
const resolved = new Map<string, discord.WebhookClient>();
const failed = new Map<string, number>();
const inflight = new Map<string, Promise<discord.WebhookClient | null>>();

const isOwnWebhook = (webhook: discord.APIWebhook, client: discord.Client): boolean => {
	if (!webhook.token) return false;
	const applicationId = client.application?.id ?? client.user?.id;
	return webhook.application_id === applicationId || webhook.user?.id === client.user?.id;
};

const discover = async (client: discord.Client, channelId: string): Promise<discord.WebhookClient | null> => {
	const existing = (await client.rest.get(discord.Routes.channelWebhooks(channelId))) as discord.APIWebhook[];
	const own = existing.find((webhook) => isOwnWebhook(webhook, client));
	if (own?.token) return new discord.WebhookClient({ id: own.id, token: own.token });

	const created = (await client.rest.post(discord.Routes.channelWebhooks(channelId), { body: { name: `${client.user?.username || 'Pepper'} Logs` } })) as discord.APIWebhook;

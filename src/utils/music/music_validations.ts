...
class VoiceChannelValidator {
    ...
    private createErrorEmbed = (message: string): discord.EmbedBuilder =>
        new discord.EmbedBuilder().setColor(this.errorColor).setDescription(message || ' ');
    ...
    public async validateVoiceConnection(): Promise<[boolean, discord.EmbedBuilder]> {
        ...
        if (!voiceChannel) {
            return [false, this.createErrorEmbed('You need to be in a voice channel')];
        }

        const botMember = this.interaction.guild?.members.me;
        if (!botMember?.permissions.has(this.requiredPermissions)) {
            return [false, this.createErrorEmbed(
    ...
    public async validateMusicSource(query: string): Promise<[boolean, discord.EmbedBuilder]> {
        return this.ytLinks.some(link => query.includes(link))
            ? [false, this.createErrorEmbed('We do not support YouTube links or music at this time :(')]
            : [true, this.createErrorEmbed('')];
    }

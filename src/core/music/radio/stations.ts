...
export const CURATED_STATIONS: readonly RadioStation[] = [
...
export const findCuratedById = (id: string): RadioStation | null => CURATED_STATIONS.find((station) => station.id === id) ?? null;

export const searchCurated = (query: string, countryCode?: string | null): RadioStation[] => {
	const q = normalize(query);
	if (!q) return [...CURATED_STATIONS];

	const cc = countryCode ? normalize(countryCode) : null;
	const scored = CURATED_STATIONS.map((station) => {
		...
		else if (genre === q) score = 50;
		else if (genre.includes(q)) score = 30;
		else if (station.country && normalize(station.country) === q) score = 20;

		if (score > 0 && cc && station.country && normalize(station.country) === cc) score += 5;

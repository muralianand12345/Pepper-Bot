...
export class SpotifyAutoComplete {
    ...
    constructor(client: discord.Client, clientId: string, clientSecret: string) {
        this.client = client;
    ...
    private getMetadataFromUrl = async (url: string): Promise<discord.ApplicationCommandOptionChoiceData> => {
        ...
        if (!urlInfo) return { name: url, value: url };
        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response?.status === 404) return { name: "Invalid Spotify URL", value: url };

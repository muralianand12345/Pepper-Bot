...
const event: BotEvent = {
    ...
    execute: async (oldState: discord.VoiceState, newState: discord.VoiceState, client: discord.Client): Promise<void> => {
        ...
        if (memberCount === 1 && player.paused) {
            player.pause(false);
            const embed = new MusicResponseHandler(client).createInfoEmbed("▶️ Resumed playback");
            await sendTempMessage(textChannel, embed);
        }

        if (memberCount === 0 && !player.paused && player.playing) {
            player.pause(true);
            const embed = new MusicResponseHandler(client).createInfoEmbed("⏸️ Paused playback because the voice channel is empty");
            await sendTempMessage(textChannel, embed);

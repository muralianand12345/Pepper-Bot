...
const processingInteractions: discord.Collection<string, boolean> = new discord.Collection();
...
const sendErrorReply = async (client: discord.Client, interaction: discord.Interaction, messageKey: string, data?: Record<string, string | number>): Promise<void> => {
    ...
    try {
        ...
        await interaction.reply({ embeds: [new MusicResponseHandler(client).createErrorEmbed(messageKey)], flags: discord.MessageFlags.Ephemeral }).catch(() => { });
    }
};
    ...
    try {
        ...
        if (!interaction.replied && !interaction.deferred) {
            ...
            try {
                ...
                await interaction.editReply({ embeds: [new MusicResponseHandler(client).createErrorEmbed(message, locale)] });
            } catch (editError) {
